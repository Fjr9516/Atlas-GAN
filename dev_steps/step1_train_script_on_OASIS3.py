...
parser.add_argument('--dataset', type=str, default='OASIS3_single_cohort')
...
# my CLI arguments
parser.add_argument('--ckpt_save_path', type=str, default='/proj/outputs/Atlas-GAN/')
parser.add_argument('--save_freq_epc', type=int, default=20)
parser.add_argument('--split_csv', type=str, default='FS_HC_scans_metadata_with_split.csv')
...
save_freq_epc = args.save_freq_epc
restore_epochs = args.restore_epochs
...
save_folder = (
    ...
    .format(exp_name, dataset, restore_epochs, atlas_model, norm_reg, lr_g, lr_d,
...
checkpoint_dir = ckpt_save_path + '/training_checkpoints/{}/'.format(save_folder)
...
if resume_ckpt > 0:
    print(f"restore from {ckpt_save_path + '/training_checkpoints/{}/ckpt-{}'.format(save_folder, resume_ckpt)}")
    checkpoint.restore(
        ckpt_save_path + '/training_checkpoints/{}/ckpt-{}'.format(save_folder, resume_ckpt)
    ).assert_consumed()

# Set up folder for tensorboard logs:
summary_writer = tf.summary.create_file_writer(
    ckpt_save_path + "logs/fit/" +
    '{}'.format(save_folder) +
...
def gen_train_step(input_images, avg_input, input_condns, epoch):
    ...
    tb_img = {
        'atlas/1': tf.nn.relu(sharp_atlases[:, 108, :, :, :]) / atlasmax,
        'atlas/2': tf.nn.relu(sharp_atlases[:, :, 95, :, :]) / atlasmax,
        'atlas/3': tf.nn.relu(sharp_atlases[:, :, :, 86, :]) / atlasmax,
        'moved_atlases/1': tf.nn.relu(moved_atlases[:, 108, :, :, :])/movedmax,
        'moved_atlases/2': tf.nn.relu(moved_atlases[:, :, 95, :, :])/movedmax,
        'moved_atlases/3': tf.nn.relu(moved_atlases[:, :, :, 86, :])/movedmax,
        'target_images/1':
            tf.image.convert_image_dtype(
                input_images[:, 108, :, :, :], dtype=tf.uint8,
            ),
        'target_images/2':
            tf.image.convert_image_dtype(
                input_images[:, :, 95, :, :], dtype=tf.uint8,
            ),
        'target_images/3':
            tf.image.convert_image_dtype(
                input_images[:, :, :, 86, :], dtype=tf.uint8,
            ),
    }
...
def disc_train_step(
    ...
    Discriminator training step. Args:
        ...
        avg_input: tf tensor of linear average repeated 'batch_size' times.
        real_images: tf tensor of training images for discriminator.
