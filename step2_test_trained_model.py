...
# outputs = [moved_atlas, diff_field_ms, new_atlas, diff_field]
...
# predict HC/AD templates
# need to focus on sharp_atlases only, and ages from 60~80 maybe
# input_condns ~ [1,3] array
# where the first value = age/97.1726095890411
# the second and third ones: 1.,0. for HC, 0.,1. for AD
...
for i, age in enumerate(range(60,81)):# 60,91
    if dataset == 'OASIS3_single_cohort':
        ...
        input_condns_HC = np.expand_dims(np.array((age / 97.1726095890411, 1.0, 0.0)),axis=0)
        input_condns_AD = np.expand_dims(np.array((age / 97.1726095890411, 0.0, 1.0)), axis=0)
    ...
    if save_path:
        ...
        if dataset == 'OASIS3_single_cohort':
            ...
            predict_and_save(input_condns_HC, 0, save_path)
            predict_and_save(input_condns_AD, 1, save_path)
    else:
        if dataset == 'OASIS3_single_cohort':
            predict_and_save(input_condns_HC, 0, './my_plot/')
        else:
            predict_and_save(input_condns_HC, 0, './my_plot/')
            predict_and_save(input_condns_AD, 1, './my_plot/')